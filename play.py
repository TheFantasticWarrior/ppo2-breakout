...
def main():
    ...
    low = np.repeat(env.observation_space.low, 4, axis=-1)
    stackedobs = np.zeros((1,)+low.shape, low.dtype)
    x,_=env.reset()
    stackedobs[..., -x.shape[-1]:] = x
    try:
        while True:
            action,_,_=m.step(tf.constant(stackedobs))
            ...
            if done:
                x,info=env.reset()
                if info:
                    print(f"episode reward:{info['rew']} length:{info['len']}")
                stackedobs[...] = 0
            stackedobs[..., -x.shape[-1]:] = x
